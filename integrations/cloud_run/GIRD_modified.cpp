...
// Include Cloud Run / Google Sheets helper (MQL5)
#include "integrations/cloud_run/mql/GIRD_GoogleSheets.mq5"
...
int OnInit() {
   ...
   // notify monitor: EA started on this terminal/account
   GGS_SendEvent("start", _Symbol);
...
void OnDeinit(const int reason) {
   // notify monitor: EA stopping
   GGS_SendEvent("stop", _Symbol);
...
void ExecuteCloseAllStrategy() {
   ...
   // notify monitor about close_all event
   GGS_SendEvent("close_all", _Symbol);
}
...
void UpdateDashboardData() {
   ...
   // send heartbeat (throttled inside helper)
   GGS_SendHeartbeat(MAGIC_NUMBER, _Symbol);
