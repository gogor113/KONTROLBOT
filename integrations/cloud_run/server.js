/*
GIRD monitor server - enhanced
- serves static UI from ./web
- CORS enabled
- optional HMAC verification (x-signature)
- SSE endpoint for live updates
- close_account action endpoint (dry-run)
*/
...
app.use(bodyParser.json({ limit: '256kb', verify: function(req, res, buf){ req.rawBody = buf; } }));
...
// CORS - allow UI origin (default allow all origins for flexibility)
app.use(cors({ origin: true, methods: ['GET','POST','OPTIONS'], allowedHeaders: ['Content-Type','x-api-key','x-signature'] }));
...
const instances = new Map(); // account -> {firstSeen,lastSeen,payload,status}
...
async function upsertMonitorRow(payload){ const sheets = await getSheets(); const header = await ensureHeaders(); const account = payload.account_login || payload.account; if(!account){ const rowObj = buildMonitorRowObject(payload, 'unknown'); const values = header.map(h => rowObj[h] !== undefined ? rowObj[h] : ''); await sheets.spreadsheets.values.append({ spreadsheetId: SHEET_ID, range:`${MONITOR_SHEET_NAME}!A1`, valueInputOption:'RAW', insertDataOption:'INSERT_ROWS', requestBody:{ values: [values] } }); return {appended:true}; }
  ...
  const resp = await sheets.spreadsheets.values.get({ spreadsheetId: SHEET_ID, range: readRange }); const values = resp.data.values || []; const accountColIndex = header.indexOf('account_login'); let foundRowIndex = -1; for(let r=0;r<values.length;r++){ const row = values[r]; if(accountColIndex>=0 && row[accountColIndex] && String(row[accountColIndex])===String(account)){ foundRowIndex = 2 + r; break; } }
  const rowObj = buildMonitorRowObject(payload, 'online'); const out = header.map(h => rowObj[h] !== undefined ? rowObj[h] : ''); if(foundRowIndex > 0){ const endCol = String.fromCharCode(65 + out.length -1); const range = `${MONITOR_SHEET_NAME}!A${foundRowIndex}:${endCol}${foundRowIndex}`; await sheets.spreadsheets.values.update({ spreadsheetId: SHEET_ID, range, valueInputOption:'RAW', requestBody:{ values:[out] } }); return {updated:true, row: foundRowIndex}; } else { await sheets.spreadsheets.values.append({ spreadsheetId: SHEET_ID, range: `${MONITOR_SHEET_NAME}!A1`, valueInputOption:'RAW', insertDataOption:'INSERT_ROWS', requestBody:{ values:[out] } }); return {appended:true}; } }
...
// SSE endpoint for instances
app.get('/sse/instances', (req, res) => {
  // allow only if API key provided (in query or header) to avoid open streaming
  const incomingKey = (req.query.key || req.get('x-api-key') || '').trim();
  if(!incomingKey || incomingKey !== API_KEY){ res.status(401).end('unauthorized'); return; }
  ...
  const send = () => {
    try{
      const payload = Array.from(instances.entries()).map(([a,o])=>({account:a, status:o.status, lastSeen:o.lastSeen, payload:o.payload}));
...
app.get('/v1/instances', (req, res) => {
  ...
  const list = []; const now = nowMs(); for(const [account,obj] of instances.entries()){ const isActive = (now - obj.lastSeen) <= HEARTBEAT_TTL*1000; if(activeOnly && !isActive) continue; list.push({ account, status: obj.status, lastSeen: new Date(obj.lastSeen).toISOString(), payload: obj.payload }); } res.json({ ok:true, instances: list });
});
...
// Close account action (dry-run by default)
app.post('/v1/close_account', async (req, res) => {
  try{
    const incomingKey = (req.get('x-api-key')||'').trim(); if(!incomingKey || incomingKey !== API_KEY) return res.status(401).json({ok:false, error:'invalid_api_key'});
    const { account, dryRun=true } = req.body || {};
    if(!account) return res.status(400).json({ok:false, error:'missing_account'});
    // record request in Events sheet
    await withRetries(()=>appendEventRow({ account_login: account, event: 'request_close', extra: { dryRun } }));
    // For safety, we don't execute trading close here (EA handles close_all logic). This endpoint signals intent.
    return res.json({ ok:true, account, dryRun });
  }catch(err){ console.error('close_account error', err); return res.status(500).json({ ok:false, error: String(err) }); }
});

// janitor to mark stale
async function janitorTick(){ try{ const now = nowMs(); const toMark = []; for(const [account,obj] of instances.entries()){ if(obj.status === 'online' && now - obj.lastSeen > HEARTBEAT_TTL*1000){ obj.status = 'offline'; instances.set(account, obj); toMark.push({ account, payload: obj.payload }); } } if(toMark.length > 0){ for(const itm of toMark){ try{ await withRetries(()=>upsertMonitorRow({ ...itm.payload, event: 'stale' })); }catch(e){ console.error('Janitor upsert failed for', itm.account, e && e.message ? e.message : e); } } } }catch(e){ console.error('Janitor error', e); } }
